...
| # | Criterion | Status | Evidence |
|---|---|---|---|
| U7 | Basecamp app GUI: local build instructions + downloadable assets + loadable in Basecamp | 🟡 | **Manual build path verified live 2026-05-22**: `brew install qt` + `cmake -DCMAKE_PREFIX_PATH=$(brew --prefix qt)` + `cmake --build .` produces `whistleblower.dylib` (240 KB, exports `WhistleblowerPlugin` + `WhistleblowerBackend`, Q_INVOKABLE table populated, embedded metadata.json matches the manifest). Framework path (LOGOS_MODULE_BUILDER_ROOT + Nix bundler → `.lgx`) needs the logos-cpp-sdk + Nix dev shell — local dev box doesn't have Nix yet, so .lgx artefact isn't published. The dylib itself is the same artefact Basecamp's `PluginLoader::load` calls `QPluginLoader().load()` on, modulo the LogosAPI symbol resolution. |
...
| # | Criterion | Status | Evidence |
|---|---|---|---|
| S14 | Registry deployed and tested on LEZ devnet/testnet | ✅ | **Deployed + 4 live on-chain instructions confirmed on 2026-05-22**. ProgramId hex `eaba04b9,...,174206c6`. Registry PDA `A9ewyji3THdFGqLAtAd9GkoPX9B9R6yb5LZCfWLxbAeH`. init_registry tx_hash `7aa30683...391c7b`. index_batch n=1 tx_hash `2f01e5ac...64d48a` (real CID from live storage upload). index_batch n=50 tx_hash `76fc8f2e...45d77`. Final readback: 51 entries × 129 bytes + 4-byte map header = 6583 bytes account data, **exactly matches the theoretical Borsh size** for `Registry { entries: BTreeMap<String, CidRecord> }`. Full evidence in [`docs/DEPLOYMENT.md`](DEPLOYMENT.md). |
